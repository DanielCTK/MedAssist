...
// 🔥 Thêm icon mới cho công cụ khoanh vùng (Lasso)
import { Eye, Layers, MousePointer, Pencil, Undo2, Aperture, Palette, Brush, Trash2, LassoSelect } from 'lucide-react'; 
import EyeModel3D from './EyeModel3D'; 

// --- Config (giữ nguyên, không thay đổi) ---
import eyeModelSimple from '../models/eye_model.glb'; 
import eyeModelDetailed from '../models/eye_model_detailed.glb'; 
const MODELS_CONFIG = {
  model1: { name: "Mô hình Đơn giản (Low-poly)", path: eyeModelSimple, isSimpleModel: true, mainMeshNames: ['eyee_low_Mesh'], glassMeshNames: ['glass_low_Mesh001'], wireframeColors: { main: 0xFF00FF, glass: 0x00FFFF, fill: 0xAA00AA }},
  model2: { name: "Mô hình Chi tiết (Y khoa)", path: eyeModelDetailed, isSimpleModel: false, retinaNames: ['Eye_CSReitna'], choroidNames: ['Eye_CSChoroid_CS'], scleraNames: ['Eye_CSScelra'], glassNames: ['Eye_CSCornea_CS', 'Eye_CSLens', 'Eye_CSSuspensory_ligaments', 'Eye_CSuspensory_ligaments_L'], otherNames: ['Eye_CSLateral_rectus', 'Eye_CSSuperior_inferior_rectus', 'Muscle_', 'Artery_', 'Vein_', 'Cylinder', 'Cube'], wireframeColors: { retina: 0xFF0000, choroid: 0xFF8C00, sclera: 0xFFFFFF, glass: 0x00FFFF, other: 0x808080, fill: 0xFFFFE0 }}
};
const DRAW_COLORS = [
...
const VisualAnalysis = ({ diagnosisData, isExpanded }) => {
    ...
    const lesionData = diagnosisData?.lesionData || [];

    // 🔥 Thêm 'region' vào các mode có thể có
    const [toolMode, setToolMode] = useState('navigate'); // 'navigate', 'draw', hoặc 'region'
    ...
    return (
        <div className="bg-white p-6 rounded-2xl shadow-lg animate-fade-in">
            ...
            <EyeModel3D 
                key={activeModelKey} config={currentModelConfig} showCornea={showCornea} 
                diagnosisLesionData={lesionData} toolMode={toolMode} lines={lines}
                setLines={setLines} drawColor={drawColor} lineWidth={lineWidth}
            />
            ...
            <div className="mt-6 p-4 bg-gray-50 rounded-xl border border-gray-200 flex justify-between items-center">
                ...
                {diagnosisData && (
                    <div className="text-sm p-2 bg-blue-50 text-blue-800 rounded-lg">
                        Chẩn đoán liên quan: **{diagnosisData.label}**
                    </div>
                )}
            </div>
        </div>
